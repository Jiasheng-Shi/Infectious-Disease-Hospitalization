...

{
  ...
  V3compOmegaUO1<-function(x){
    
    omegafi=c()
    for (i in 1:24) {
      omegafi[i]=pgamma(i,shape = x[1],scale = x[2])-pgamma((i-1),shape = x[1],scale = x[2])
    }
    omegafi[25]=pgamma(24,shape = x[1],scale = x[2],lower.tail = FALSE)
    
    result=-CompLike(paraTOutput=simuRes1$estimate,paraT=simuRes1$estimate,
                     I=I,
                     H=H,
                     Z=Z,
                     omegaOutput=omegafi,
  ...
  timeS=Sys.time()
  simuomega1=optim(fn=V3compOmegaUO1,par = c(2,5), 
                   lower = rep(0.1,2),upper = rep(15,2),
                   method = "L-BFGS-B",
                   #control = list(pgtol=0 )
  )
  Sys.time()-timeS
  ...
  Omega1=c()
  for (i in 1:24) {
    Omega1[i]=pgamma(i,shape = simuomega1$par[1],scale = simuomega1$par[2])-
      pgamma((i-1),shape = simuomega1$par[1],scale = simuomega1$par[2])
  }
  Omega1[25]=pgamma(24,shape = simuomega1$par[1],scale = simuomega1$par[2],lower.tail = FALSE)
...

{
  ...
  V3compOmegaUO2<-function(x){
    ...
    result=-CompLike(paraTOutput=simuRes2$estimate,paraT=simuRes2$estimate,
                     I=I,
                     H=H,
                     Z=Z,
                     omegaOutput=omegafi,
  ...
  timeS=Sys.time()
  simuomega2=optim(fn=V3compOmegaUO2,par = simuomega1$par, 
                   lower = rep(0.1,2),upper = rep(15,2),
                   method = "L-BFGS-B",
                   #control = list(pgtol=0 )
  )
  ...
  Omega2=c()
  for (i in 1:24) {
    Omega2[i]=pgamma(i,shape = simuomega2$par[1],scale = simuomega2$par[2])-
      pgamma((i-1),shape = simuomega2$par[1],scale = simuomega2$par[2])
  }
  Omega2[25]=pgamma(24,shape = simuomega2$par[1],scale = simuomega2$par[2],lower.tail = FALSE)
...

{
  ...
  V3compOmegaUO3<-function(x){
    ...
    result=-CompLike(paraTOutput=simuRes3$estimate,paraT=simuRes3$estimate,
                     I=I,
                     H=H,
                     Z=Z,
                     omegaOutput=omegafi,
  ...
  timeS=Sys.time()
  simuomega3=optim(fn=V3compOmegaUO3,par = simuomega2$par, 
                   lower = rep(0.1,2),upper = rep(15,2),
                   method = "L-BFGS-B",
                   #control = list(pgtol=0 )
  )
  ...
  Omega3=c()
  for (i in 1:24) {
    Omega3[i]=pgamma(i,shape = simuomega3$par[1],scale = simuomega3$par[2])-
      pgamma((i-1),shape = simuomega3$par[1],scale = simuomega3$par[2])
  }
  Omega3[25]=pgamma(24,shape = simuomega3$par[1],scale = simuomega3$par[2],lower.tail = FALSE)
...
#saveRDS(list(simuRes1,simuRes2,simuRes3,simuResIni,I,H,Z,simuomega1,simuomega2,simuomega3,tildeomegaIni), file='HosCovidCase3.RDS')
  ...
  OResRepUO[[Simrep]]=list(simuRes1,simuRes2,simuRes3,simuResIni,I,H,Z,simuomega1,simuomega2,simuomega3)
  
}

#Sys.time()-timerep

#saveRDS(simuomega1,file = 'omegaUOtrial1.RDS')

saveRDS(OResRepUO,file=paste0('HosCovidCase3UO', '_irep', irep, '.RDS'))
