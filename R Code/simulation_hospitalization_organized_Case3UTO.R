...

{
  ...
  VTcompTOmega1=function(x){
    
    tildeomegafi=c(x[1:6],1-sum(x[1:6]))
    if (tildeomegafi[7]<0){
        penalty=-tildeomegafi[7]
        tildeomegafi[7]=0
    }else{
        penalty=0
    }
    
    result=-CompLike(paraTOutput=paraT1,paraT=paraT1,
                     I=I,
                     H=H,
                     Z=Z,
                     omegaOutput=Omega1,
                     tildeomegaOutput=tildeomegafi,
                     omegaInput=Omega1,
                     tildeomegaInput=tildeomegaIni)$cellQLV
    result=result+100000*penalty
    return(result)
  ...
  timeS=Sys.time()
  simuTildeomega1=optim(fn=VTcompTOmega1,par = tildeomegaIni[1:6], 
                   lower = rep(0.001,6),upper = rep(0.999,6),
                   method = "L-BFGS-B",
                   #control = list(pgtol=0 )
  )
  Sys.time()-timeS
  ...
  simuTildeomega1$par
  tildeomega1=c(simuTildeomega1$par,1-sum(simuTildeomega1$par))
...

{
  ...
  VTcompTOmega2=function(x){
    ...
    result=-CompLike(paraTOutput=paraT2,paraT=paraT2,
                     I=I,
                     H=H,
                     Z=Z,
                     omegaOutput=Omega2,
  ...
  timeS=Sys.time()
  simuTildeomega2=optim(fn=VTcompTOmega2,par = tildeomega1[1:6], 
                        lower = rep(0.001,6),upper = rep(0.999,6),
                        method = "L-BFGS-B",
                        #control = list(pgtol=0 )
  )
  ...
  simuTildeomega2$par
  tildeomega2=c(simuTildeomega2$par,1-sum(simuTildeomega2$par))
...

{
  ...
  VTcompTOmega3=function(x){
    ...
    result=-CompLike(paraTOutput=paraT3,paraT=paraT3,
                     I=I,
                     H=H,
                     Z=Z,
                     omegaOutput=Omega3,
  ...
  timeS=Sys.time()
  simuTildeomega3=optim(fn=VTcompTOmega3,par = tildeomega2[1:6], 
                        lower = rep(0.001,6),upper = rep(0.999,6),
                        method = "L-BFGS-B",
                        #control = list(pgtol=0 )
  )
  ...
  simuTildeomega3$par
  tildeomega3=c(simuTildeomega3$par,1-sum(simuTildeomega3$par))
...
#saveRDS(list(simuRes1,simuRes2,simuRes3,simuResIni,I,H,Z,simuomega1,simuomega2,simuomega3,tildeomegaIni), file='HosCovidCase3.RDS')
  ...
  OResRepUTO[[Simrep]]=list(simuRes1,simuRes2,simuRes3,simuResIni,I,H,Z,
                         simuomega1,simuomega2,simuomega3,
                         simuTildeomega1,simuTildeomega2,simuTildeomega3)
  
}

#Sys.time()-timerep

saveRDS(OResRepUTO,file=paste0('HosCovidCase3UTO', '_irep', irep, '.RDS'))
